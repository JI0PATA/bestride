...
{
    if ($status === 0) $status_class = 'alert-danger';
    else if ($status === 1) $status_class = 'alert-success';

    session()->flash('popup_msg', "
    <div id='popup_msg' class='{$status_class}'>
    {$msg}
    </div>
    ");
}
...
{
    $be = explode('/', $date);
    $birthdate = $be[2] . '-' . $be[1] . '-' . $be[0];
    return $birthdate;
}

function getAge($birthdate)
{
    $be = explode('-', $birthdate);
    $age = Carbon::createFromDate($be[0], $be[1], $be[2])->age;
    return $age;
}